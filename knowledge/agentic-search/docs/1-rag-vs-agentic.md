...
| --- | --- | --- |
| Code, configs, runbooks, structured docs | Agentic | Exact symbols and identifiers matter |
| FAQs, customer-support tickets, marketing content | RAG | Phrasing varies; semantic match wins |
| Internal wiki, engineering knowledge base | Agentic | Exact terms, multi-hop questions |
| Books, papers, long-form articles | RAG | Semantic similarity matters more than exact match |
| "What was the decision we made about X?" | Agentic | The agent can chase references between docs |
| "Show me anything similar to this user's complaint" | RAG | Vector similarity is the whole point |

## Where these recommendations come from

The shift toward agentic search for code is reflected in how every major coding agent works in 2026, and there are public numbers to back it up.
...
**Stop reaching for a vector database by default.** For many of the cases that show up in real production systems, especially anything code- or runbook-shaped, agentic search with grep is simpler, fresher, and more honest about what it found.
